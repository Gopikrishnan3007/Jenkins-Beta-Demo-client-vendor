...
public class VendorDaoImplementation implements VendorDao {
	...
	public Vendor save(VendorDto vendor) throws MessagingException {
		...
		tpc.setRole(role);
		tpc.setIsPasswordChanged(false);
	...
	public Long getCountOfCandidateByVendorId(Long vendorId) {
		String hql = "select count(c) from Candidate c where c.source = 'VENDOR' and c.sourceId = :vendorId";
		String hq = "select c from Candidate c join MRFVendor m on c.source='VENDOR' and c.sourceId = m.vendor.vendorId "
				+ "where c.source = 'VENDOR' and c.sourceId = :vendorId";
		Long q = (Long) entityManager.createQuery(hql).setParameter("vendorId", vendorId).getSingleResult();
 
		List<Candidate> q1 = (List<Candidate>) entityManager.createQuery(hq).setParameter("vendorId", vendorId)
				.getResultList();
		int count = 0;
		for (Candidate c : q1) {
...
//				+ "WHERE c.source = 'VENDOR' AND c.sourceId = :vendorId AND (c.status = 'selected')";
//		Query q = entityManager.createQuery(hql);
//		q.setParameter("vendorId", vendorId);
...
//	}
	...
	public List<Candidate> getHiredAndJoinedCandidatesAssignedByVendorId(Long vendorId) {
		String hql = "select c FROM Candidate c JOIN MRFVendor m ON c.sourceId = m.vendor.vendorId "
				+ "WHERE c.source = 'VENDOR' AND c.sourceId = :vendorId AND (c.status IN ('SELECTED', 'HIRED','OFFERED','OFFER ACCEPTED') OR c.status = 'JOINED')";
		Query q = entityManager.createQuery(hql);
		q.setParameter("vendorId", vendorId);
		return q.getResultList();
	}
...
//				+ "WHERE c.source = 'VENDOR' AND c.sourceId = :vendorId AND c.status = 'SCREENED'";
//		Query q = entityManager.createQuery(hql);
//		q.setParameter("vendorId", vendorId);
//		return (Long) q.getSingleResult();
//	}
...
//		String hql = "SELECT COUNT(c) FROM Candidate c JOIN MRFVendor m ON c.sourceId = m.vendor.vendorId"
		String hql = "SELECT COUNT(c) FROM Candidate c "
				...
				+ "WHERE c.source = 'VENDOR' AND c.sourceId = :vendorId AND c.status IN ('SELECTED', 'HIRED','OFFERED','OFFER ACCEPTED')";
		Query q = entityManager.createQuery(hql);
		q.setParameter("vendorId", vendorId);
		return (Long) q.getSingleResult();
	}
	...
	public Long getCountOfJoinedCandidateByVendorId(Long vendorId) {
		String hql = "select count(c) from Candidate c "
				+ "where c.source = 'VENDOR' AND c.sourceId = :vendorId AND c.status IN ( 'JOINED')";
		Query q = entityManager.createQuery(hql);
		q.setParameter("vendorId", vendorId);
		return (Long) q.getSingleResult();
	}
	...
	public List<MRFVendor> findAllMRFvendorDetailsByRmId(Long rmId) {
		String hql = "SELECT m FROM MRFVendor m WHERE m.recruitingManager.employeeId = :rmId and m.vendorAssignedStatus = 'ASSIGNED'";
		Query q = entityManager.createQuery(hql);
	...
	public List<Candidate> getAllCandidatesAssignedByVendorAndMrfJdId(Long vendorId, Long mrfJdId) {
		String hql = "select c from Candidate c join c.MrfJd m where c.source = 'VENDOR' and c.sourceId = :vendorId and m.mrfJdId = :mrfJdId";
		Query q = entityManager.createQuery(hql);
		q.setParameter("vendorId", vendorId);
		q.setParameter("mrfJdId", mrfJdId);
		return q.getResultList();
	}
...
//		String hql = "SELECT MONTH(c.assignedAt) AS month, COUNT(c.candidateId)  AS hiredCount " + "FROM Candidate c"
//				+ " WHERE c.sourceId = :vendorId AND c.source = 'VENDOR' AND YEAR (c.assignedAt) = :year AND c.status ='Joined' "
//				+ "GROUP BY MONTH(c.assignedAt) " + "ORDER BY MONTH(c.assignedAt)";
//		
		String hql = "SELECT MONTH(c.assignedAt) AS month, "
				+ "COUNT(CASE WHEN c.status = 'selected' THEN c.candidateId END) AS hiredCount, "
				+ "COUNT(c.candidateId) AS totalCount " + "FROM Candidate c "
				+ "WHERE c.sourceId = :vendorId AND c.source = 'VENDOR' AND YEAR(c.assignedAt) = :year "
				+ "GROUP BY MONTH(c.assignedAt) " + "ORDER BY MONTH(c.assignedAt)";
		return entityManager.createQuery(hql).setParameter("vendorId", vendorId).setParameter("year", year)
				.getResultList();
	...
	public Long getRecruitingManagerId(Long vendorId, Long mrfId) {
		String hql = "SELECT mv.recruitingManager.employeeId FROM MRFVendor mv WHERE mv.vendor.vendorId = :vendorId AND mv.mrf.mrfId = :mrfId";
		Long employeeId = (Long) entityManager.createQuery(hql).setParameter("vendorId", vendorId)
				.setParameter("mrfId", mrfId).getSingleResult();
		return employeeId;
	}
